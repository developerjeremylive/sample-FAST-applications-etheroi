...
"""
Cross-platform frontend deployment script for Dual Monitoring.
...
def get_secret_value(secret_arn: str) -> str:
    """
    Retrieve a secret string value from Secrets Manager via AWS CLI.
...
def generate_aws_exports(
    ...
    # Add DevOps Agent webhook config if available.
    # Priority: existing aws-exports.json values > CDK-derived values.
    # This allows the webhook URL and secret to be manually overridden after
    # the Console webhook is created without being clobbered on each deploy.
    existing_exports: Dict[str, str] = {}
    existing_path = frontend_dir / "public" / "aws-exports.json"
    if existing_path.exists():
        try:
            existing_exports = json.loads(existing_path.read_text())
        except Exception:
            pass

    devops_incident_api_url = outputs.get("DevOpsIncidentApiUrl")
    if devops_incident_api_url:
        aws_exports["devopsIncidentApiUrl"] = devops_incident_api_url
        log_success(f"DevOps incident API URL: {devops_incident_api_url}")

    devops_webhook_url = (
        existing_exports.get("devopsWebhookUrl")
        or outputs.get("DevOpsWebhookUrl")
    )
    if devops_webhook_url:
        aws_exports["devopsWebhookUrl"] = devops_webhook_url
        log_success(f"DevOps webhook URL: {devops_webhook_url}")

    devops_webhook_secret = existing_exports.get("devopsWebhookSecret")
    if devops_webhook_secret:
        aws_exports["devopsWebhookSecret"] = devops_webhook_secret
        log_success("DevOps webhook secret preserved from existing aws-exports.json")
    elif outputs.get("DevOpsWebhookSecretArn"):
        log_info("Fetching DevOps webhook secret from Secrets Manager...")
        try:
            devops_webhook_secret = get_secret_value(outputs["DevOpsWebhookSecretArn"])
            aws_exports["devopsWebhookSecret"] = devops_webhook_secret
            log_success("DevOps webhook secret retrieved from Secrets Manager")
        except subprocess.CalledProcessError as e:
            log_warning(f"Could not retrieve DevOps webhook secret: {e.stderr}. Skipping.")
