...
This guide walks you through deploying the Dual Monitoring Agent Solution to AWS.
...
backend:
  pattern: strands-swarm-agent # Available patterns: strands-single-agent, langgraph
...
This repo supports two deployment types for AgentCore Runtime. Set `deployment_type` in `infra-cdk/config.yaml`:
...
After the main stack is deployed, set up the AWS DevOps Agent for autonomous infrastructure investigation. This is a separate deployment that connects to the proxy Lambda already included in the main stack.

See the [DevOps Agent Setup Guide](DEVOPS_AGENT_SETUP.md) for complete instructions covering:
- Deploying the DevOps Agent Space
- Creating the event channel webhook
- Storing webhook credentials in SSM and Secrets Manager
- Configuring the proxy Lambda
